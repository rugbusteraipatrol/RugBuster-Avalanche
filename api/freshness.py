...
Ported from the Solana service, where `/score` returned a collector row of any
age as a current verdict. **The same defect is not live here**, and saying so
matters more than applying the same patch: AVAX `/score` serves only the
180-second in-memory cache and otherwise recomputes. `lookup_cached_score`
does query `avax_scans` with no age bound, but it is dead code -- defined and
never called -- and it does not even select `created_at`, so it could not age a
row if it tried. It is corrected here so that wiring it up later cannot
reintroduce the Solana failure.

What was genuinely missing on this service is the reporting: a cached response
did not say when it was computed, so a caller could not tell a fresh verdict
from one at the end of its window.
...
# The in-memory score cache's window. Mirrors SCAN_CACHE_TTL_SECONDS so one
# definition of "current" covers both the cache check and what is reported.
MEMORY_CACHE_MAX_AGE = timedelta(seconds=180)

# Bound for the stored-scan path. Unused today (see the module note), set so
# that wiring it up cannot serve evidence of arbitrary age.
STORED_SCAN_MAX_AGE = timedelta(hours=24)
...
# module was written for a response shaped `label`/`risk_score` and then reused
# where the verdict also lives in `rug_score`, `rug_status` and the speculation
# pair. Blanking only the first two left a caller reading `rug_score` the stale
# number the withholding was supposed to withhold.
#
# Listing names that a given service does not use is harmless -- absent keys are
...
# What a withheld field becomes. `label` gets a word because callers switch on
# it; the numeric and status fields go to None so that "no opinion" cannot be
...
def assess(observed_at: Any, max_age: timedelta = MEMORY_CACHE_MAX_AGE,
